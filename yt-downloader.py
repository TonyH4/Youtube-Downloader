...
def menu(prompt: str, options: "list of pairs(string, function); string should contain exactly one '&' preceding the key letter"):
    switch = dict()
    if prompt:
        prompt += '\n'
    else: prompt = ''
...
def choose_from_my():
    if youtube.login.username:
        ...
        for (i, playlist) in enumerate(playlists):
            print("{0}. : {1}({2} songs) - {3}".format(i + 1, playlist['title'], playlist['count'], playlist['privacy']))
...
def download():
    menu("What do you want to download?",[("&Song", dl_video),
                                          ("&Playlist", functools.partial(menu, None, [
                                              ...
                                              #("M3&U Playlist",)
                                              ("&My youtube playlists [" + youtube.login.username + "]", choose_from_my) if youtube.login.username else
    ...
    try:
        if len(sys.argv) == 1:
            ...
            while menu("What do you want to do?",[("&Download", download),
                                                  #("&Convert", convert),
                                                  ("&Logout [logged in as " + youtube.login.username + "]", youtube.logout) if youtube.login.username else
                                                  ("&Login", login)]): pass
            return 0
